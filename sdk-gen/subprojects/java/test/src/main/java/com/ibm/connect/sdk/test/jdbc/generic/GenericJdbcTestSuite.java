...
/*                                                     */
/* *************************************************** */
package com.ibm.connect.sdk.test.jdbc.generic;
...
/**
 * An abstract class for a generic JDBC test suite.
 */
public abstract class GenericJdbcTestSuite extends ConnectorTestSuite
{
    private static ModelMapper modelMapper = new ModelMapper();

    protected abstract Connection getConnection();

    protected abstract ConnectionProperties createDerbyConnectionProperties();

    protected ConnectionProperties createConnectionProperties(ConnectionProperties derbyConnectionProperties)
    {
        final ConnectionProperties connectionProperties = new ConnectionProperties();
        connectionProperties.put("jdbc_url", DerbyUtils.getConnectionURL(derbyConnectionProperties));
        connectionProperties.put("username", derbyConnectionProperties.get("username"));
        connectionProperties.put("password", derbyConnectionProperties.get("password"));
        connectionProperties.put("row_limit_support", "suffix");
        connectionProperties.put("row_limit_suffix", "FETCH FIRST ${row_limit} ROWS ONLY");
    ...
    /**
     * Test getStream with row_limit using driver default.
     *
     * @throws Exception
     */
    @Test
    public void testGetStreamRowLimitDefault() throws Exception
    {
        try (Statement statement = getConnection().createStatement()) {
            statement.execute("CREATE SCHEMA SCHEMA1");
            statement.execute("CREATE TABLE SCHEMA1.T1 (C1 INTEGER)");
        ...
        try {
            ...
            descriptor.setConnectionProperties(createConnectionProperties());
            descriptor.setInteractionProperties(interactionProperties);
            descriptor.getConnectionProperties().remove("row_limit_support");
            descriptor.getConnectionProperties().remove("row_limit_suffix");
            interactionProperties.put("schema_name", "SCHEMA1");
            interactionProperties.put("table_name", "T1");
            interactionProperties.put("row_limit", "5");
            final FlightInfo info = getClient().getInfo(FlightDescriptor.command(modelMapper.toBytes(descriptor)));
    ...
    public void testDiscoverSchemasExcludeSystem() throws Exception
    {
        ...
        try {
            ...
            // Generic JDBC doesn't recognize system schemas. All are included.
            assertTrue(schemas.contains("SYS"));
        }
        finally {
            try (Statement statement = getConnection().createStatement()) {
    ...
    public void testPartitionedGetAndPut() throws Exception
    {
        ...
        try {
            ...
            assertEquals(1, sourceInfo.getEndpoints().size());
            for (final FlightEndpoint endpoint : sourceInfo.getEndpoints()) {
                try (FlightStream stream = getClient().getStream(endpoint.getTicket()); VectorSchemaRoot root = stream.getRoot()) {
    ...
    public void testConnectionBadDatabase() throws Exception
    {
        final CustomFlightActionRequest request = new CustomFlightActionRequest();
        request.setDatasourceTypeName(getDatasourceTypeName());
        final ConnectionProperties derbyConnectionProperties = createDerbyConnectionProperties();
        derbyConnectionProperties.put("database", "unknown");
        derbyConnectionProperties.remove("create_database");
        request.setConnectionProperties(createConnectionProperties(derbyConnectionProperties));
        try {
    ...
    public void testConnectionSSL() throws Exception
    {
        ...
        request.setDatasourceTypeName(getDatasourceTypeName());
        final ConnectionProperties derbyConnectionProperties = createDerbyConnectionProperties();
        derbyConnectionProperties.put("ssl", "true");
        request.setConnectionProperties(createConnectionProperties(derbyConnectionProperties));
        try {
            getClient().doAction(new Action("test", modelMapper.toBytes(request))).next();
            fail("Exception expected");
     ...
     */
    @Test
    public void testConnectionMissingJdbcUrl() throws Exception
    {
        final CustomFlightActionRequest request = new CustomFlightActionRequest();
        request.setDatasourceTypeName(getDatasourceTypeName());
        request.setConnectionProperties(createConnectionProperties());
        request.getConnectionProperties().remove("jdbc_url");
        try {
            getClient().doAction(new Action("validate", modelMapper.toBytes(request))).next();
            fail("Exception expected");
        }
        catch (Exception e) {
            assertTrue(e.getMessage().contains("Missing jdbc_url"));
        }
    }
     ...
     */
    @Test
    public void testConnectionInvalidJdbcUrl() throws Exception
    {
        final CustomFlightActionRequest request = new CustomFlightActionRequest();
        request.setDatasourceTypeName(getDatasourceTypeName());
        request.setConnectionProperties(createConnectionProperties());
        request.getConnectionProperties().put("jdbc_url", "invalid");
        try {
            getClient().doAction(new Action("validate", modelMapper.toBytes(request))).next();
            fail("Exception expected");
        }
        catch (Exception e) {
            assertTrue(e.getMessage().contains("Invalid JDBC URL"));
        }
    }
     ...
     */
    @Test
    public void testConnectionUnknownDriver() throws Exception
    {
        final CustomFlightActionRequest request = new CustomFlightActionRequest();
        request.setDatasourceTypeName(getDatasourceTypeName());
        request.setConnectionProperties(createConnectionProperties());
        request.getConnectionProperties().put("jdbc_url", "jdbc:unknown://host/database");
        try {
            getClient().doAction(new Action("validate", modelMapper.toBytes(request))).next();
            fail("Exception expected");
        }
        catch (Exception e) {
            assertTrue(e.getMessage().contains("Driver [unknown] is not one of"));
        }
    }
